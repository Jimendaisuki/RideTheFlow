...
void CastleVaristor::Update()
{
		...
		Vector3::Distance(playerMat.GetPosition(), mPosition) <= CastleCannonAttackRange)
	{
		attackRag = 0.0f;
		arrowCount++;
		world.Add(ACTOR_ID::ENEMY_BULLET, std::make_shared<VaristorBullet>(world, mPosition + Vector3(0, 10, 0), *this, mRotateY - 90, attackAngleZ,CastleVaristorInitialVelocity,CastleVaristorArrowAccuracy));
